...
function customAssign(...objects) {
  ...
  const combinedObj = Object.assign({}, ...objects);
  // Перетворення числових значень на рядки, ітеруємо об'єкт за допомогою for in
  for (const key in combinedObj) {
    if (typeof combinedObj[key] === "number") {
      combinedObj[key] = combinedObj[key].toString();
    }
  }
...
function customObjectFromEntries(entries) {
  ...
  // Використання методу `Object.fromEntries` для створення об'єкта з масиву оброблених записів
...
function keysAndValues(obj) {
  ...
  let objectsArr = Object.keys(obj);
...
function filterObjectsByKey(arr, key) {
  ...
  let newArr = arr.filter((obj) => obj.hasOwnProperty(key));
  return newArr;
...
console.log(checkOwnership({ a: 1, b: 2, c: 3 }, { a: 1, b: 2, c: 2 }, "a")); // true
